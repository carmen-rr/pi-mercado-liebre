let db = {
    usuario:{
        email: 'gogo_boquita@sosmivida.com.bo', 
        usuario: 'Gogoxeneize2007', 
        contraseña: 'te fuiste a la b', 
        fechaDeNacimiento: '10/04/1905',
        dni: 24061978, 
        fotoDePerfil: '/images/users/fotodeperfil.jpg',
        cantidadDeProducto: 10,
        seguidores: 500,
        comentarios: 365


    },

    producto:[
        {
        imagen:'/images/products/tiburon1.jpeg', 
        nombre: 'Tiburón ', 
        descripcion: 'Disfraz de Tiburón para adultos', 
        fechaDeCarga: 'Fecha de carga: 01/04/2022', 
        }, 
        {
        imagen:'/images/products/dino1.jpeg', 
        nombre: 'Dinosaurio', 
        descripcion: 'Disfraz de dinosaurio para adultos', 
        fechaDeCarga: 'Fecha de carga: 05/03/2022', 
        },
        {
        imagen:'/images/products/panchito1.jpeg', 
        nombre: 'Pancho', 
        descripcion: 'Disfraz de pancho (Hot Dog) para perros', 
        fechaDeCarga: 'Fecha de carga: 16/09/2022', 
        },
        {
        imagen:'/images/products/sportacus1.jpeg', 
        nombre: 'Sportacus', 
        descripcion: 'Disfraz de Sportacus (Lazy Town) para adultos', 
        fechaDeCarga: 'Fecha de carga: 22/05/2022', 
        }, 
        {
        imagen:'/images/products/caballo1.jpeg', 
        nombre: 'Caballo', 
        descripcion: 'Disfraz de Caballo para adultos', 
        fechaDeCarga: 'Fecha de carga: 07/03/2022', 
        },
        {
        imagen:'/images/products/banana1.jpeg', 
        nombre: 'Banana', 
        descripcion: 'Disfraz de Banana para adultos', 
        fechaDeCarga: 'Fecha de carga: 03/09/2022', 
        },
        {
        imagen:'/images/products/yoda1.jpeg', 
        nombre: 'Baby Yoda', 
        descripcion: 'Disfraz de Yoda para bebes', 
        fechaDeCarga: 'Fecha de carga: 08/11/2022', 
        }, 
        {
        imagen:'/images/products/bob1.jpeg', 
        nombre: 'Bob Esponja', 
        descripcion: 'Disfraz de Bob Esponja para adultos', 
        fechaDeCarga: 'Fecha de carga: 02/1/2022', 
        },
        {
        imagen:'/images/products/alien1.jpeg', 
        nombre: 'Alien', 
        descripcion: 'Disfraz de Alien para adultos', 
        fechaDeCarga: 'Fecha de carga: 08/04/2022', 
        },
        {
        imagen:'/images/products/buzz1.jpeg', 
        nombre: 'Buzz Light Year', 
        descripcion: 'Disfraz de Buzz Light Year (Toy Story) para adultos', 
        fechaDeCarga: 'Fecha de carga: 23/12/2022', 
        }, 
    ],

    comentarios:[
            {
            nombreDeUsuario:'Elnicocrack123', 
            textoDeComentario: '¡Me encanto este disfraz! ¡Solo lo encontre aquí! Es un increíble negocio de disfraces, yo diría único', 
            imagenDePerfil: '/images/products/usuario1.jpeg', 
            },
            {
            nombreDeUsuario:'BrianGod', 
            textoDeComentario: 'Lo recomiendo muchísimo, además me encanta su sitio web, fácil de usar, atractivo y con amplia variedad! ', 
            imagenDePerfil: '/images/products/usuario2.jpeg', 
            },
            {
            nombreDeUsuario:'LuisTiburonDeLaGuaira', 
            textoDeComentario: 'La calidad de los disfraces es muy buena. Del estudio de mercado que hice, probablemente sea el mejor lugar de los que vi.', 
            imagenDePerfil: '/images/products/usuario3.jpeg', 
            },
            {
            nombreDeUsuario:'Velocipanchor123', 
            textoDeComentario: 'Lo recomiendo mucho, cuando necesito un disfraz siempre acudo a Forever 31', 
            imagenDePerfil: '/images/products/usuario4.jpeg', 
            },
            {
            nombreDeUsuario:'Sportacusfan2003', 
            textoDeComentario: '¡Muy buen disfraz, mucha variedad y excelente calidad!', 
            imagenDePerfil: '/images/products/usuario5.jpeg', 
            },
            {
            nombreDeUsuario:'d10sito', 
            textoDeComentario: 'Que buen disfraz! Una compra de 10 ;)', 
            imagenDePerfil: '/images/products/usuario1.jpeg', 
                },
                {
            nombreDeUsuario:'Elrockstarflogger00', 
            textoDeComentario: 'La compra llego en impecable estado! Sumamente recomendado', 
            imagenDePerfil: '/images/products/usuario2.jpeg', 
                },
                {
            nombreDeUsuario:'Popeyemarinosoy', 
            textoDeComentario: '¡Muy original!', 
            imagenDePerfil: '/images/products/usuario3.jpeg', 
                },
                {
            nombreDeUsuario:'Teletubbie007', 
            textoDeComentario: 'Nuevamente, vuelvo a elegir Forever 31', 
            imagenDePerfil: '/images/products/usuario4.jpeg', 
                },
                {
            nombreDeUsuario:'Elgranediporey', 
            textoDeComentario: '¡Me encanto, recomiendo!', 
            imagenDePerfil: '/images/products/usuario5.jpeg', 
                },
    ],  
    
}

module.exports = db;